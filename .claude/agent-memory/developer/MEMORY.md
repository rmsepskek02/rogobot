...
## GOLD_TABLE 구조 패턴 (2026-04-16)

### 3-tuple 구조 및 귀속골드 분리
- `GOLD_TABLE`은 `[최소레벨, 비귀속골드, 귀속골드]` 3-tuple, 내림차순 정렬
- `GOLD_TABLE.find(([min]) => lv >= min)`으로 해당 구간 탐색
- 귀속골드는 입장 레벨 제한이 있는 컨텐츠(지평의 성당 등) 전용 — 해당 컨텐츠 추가/변경 시 반드시 최소레벨 기준 확인
- 출력 시 귀속골드 > 0인 경우에만 귀속 줄을 추가하여 저레벨 캐릭터에서 불필요한 줄 방지

### Plan.md vs 작업 지시서 값 불일치 주의
- 오케스트레이터의 작업 지시서와 Plan.md의 구체적 수치가 다를 수 있음
- **Plan.md가 공식 문서** — 수치 충돌 시 Plan.md 기준을 우선 적용할 것
