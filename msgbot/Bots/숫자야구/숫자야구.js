...
function responseFix(room, msg, sender, isGroupChat, replier, imageDB, packageName) {
  // 방 검사
  if(room == "한용희" || room == "랜타디 로열패밀리" || room == "로아온" || room == "로아 안식처")
  {
    ...
    if (isStart == true) {
        ...
        if (isNaN(num)) {
            ...
            if(selected == true){
                ...
                if(msg == "!한번더"){
                    leftCount++;
                    replier.reply('한번 더 너에게 질풍같은 기회를!');
                }
                if(msg == "!힌트"){
                    if(isHint == false){
                        var hint = Math.floor(Math.random() * 4);
                        if(hint == 0){ 
                          replier.reply(number[hint] + "XXX");
                        } else if(hint == 1){ 
                          replier.reply("X" + number[hint] + "XX");
                        } else if(hint == 2){ 
                          replier.reply("XX" + number[hint] + "X");
                        } else if(hint == 3){ 
                          replier.reply("XXX" + number[hint]);
                        } else { 
                          replier.reply("버그다!"); 
                        }
                        isHint = true;
        ...
        // 숫자인 경우
            ...
            // 숫자가 4자리가 아닌경우 
            if (msgArr.length != 4 || numArr.length != 4) {
                return;
            }
        }
            // 난이도를 선택한 경우
            ...
            //   replier.reply(number);
                    ...
                    while (count < leftCount) {
                        ...
                        if (strike == 4) {
                            ...
                            replier.reply(numArr.join('') + ': ' + strike + 'S ' + ball + 'B\n남은기회는 '+ _leftCount + "번!!");
                            break;
                        }
                    }
            }
